...
class PatternMgr:
    ...
    def __init__(self):
        ...
        self._whitespaceRE = re.compile("\s+", re.ASCII)  # python 3.6.2
    ...
    def printLevel(self, line_width, section, n):
        ...
        if isinstance(n, dict):
            for (slug, title) in n.items():
                print("-" * line_width, slug)
                self.printLevel(line_width + 1, section, title)
        elif isinstance(n, (frozenset, list, set, tuple,)):
            for slug in n:
                self.printLevel(line_width + 1, section, slug)
        else:
            print("-" * line_width, n)
